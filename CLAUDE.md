...
- **Fiche dépanneur** = la surface de travail. Documents groupés par `niveau_exigence` (socle déplié, complémentaire replié) puis par famille à l'intérieur. Par ligne : état, date de péremption, `J-xx`, **aperçu dans l'application** (blob → `<iframe>`, le JWT interdit un `<iframe src>` direct), dépôt d'une nouvelle version, suppression, **glisser-déposer** d'un PDF sur la ligne. Les cellules grises (non applicables) ne sont pas affichées : il n'y a rien à y déposer.
  - **Un document manquant rend la ligne entière cliquable** et son bouton « ⬆ Déposer » passe en vert plein. Il n'y a rien d'autre à faire sur une ligne rouge — ni consulter, ni remplacer — donc obliger à viser un bouton au bout d'un écran de large n'avait pas de sens.
  - **L'équipe de Pérols se distingue à l'œil** (2026-08-21) : tag « Pérols » au jaune de la charte (`--c-accent`), fond crème et filet jaune à gauche de la ligne. Les deux équipes se suivent dans la même liste sans venir du même endroit, et Flotte ne connaît ni prénom, ni équipe, ni intérim — sans marqueur, on cherche pourquoi ces fiches sont plus vides que leurs voisines. La teinte reste volontairement légère : le rouge/orange/vert de la conformité doit rester le signal fort. Le site est lu dans la clé externe (`perols:12`) via `models.site_de`, pas stocké dans une colonne.
  - **La liste se trie par en-tête** (nom, conformité, qualification, manquants, périmés, expirent). Le premier clic part dans le sens utile à la colonne : conformité et qualification du plus bas au plus haut, compteurs de manques du plus haut au plus bas.
...
- **Suppression d'une pièce déposée par erreur** (`DELETE /api/documents/{version_id}`, 2026-08-21) : scan à l'envers, document rattaché au mauvais dépanneur, mauvais type choisi. Ce n'est **pas** une entorse à l'invariant ci-dessus, qui protège l'historique des *renouvellements* — une pièce qui n'est pas la bonne n'est pas une trace de conformité, et la laisser dans l'export remis à l'URSSAF serait pire que de l'effacer. Chaque suppression est tracée dans `audit_log` (type, dépanneur, nom du fichier, statut). Le fichier chiffré part avec la ligne ; le document retrouve son état d'avant (la version validée précédente redevient courante, ou le `Document` lui-même disparaît s'il n'en reste aucune, plutôt qu'une coquille vide). Confirmation en deux clics sur la ligne, sans modale.
