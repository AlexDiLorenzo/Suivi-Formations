...
- **La liste des dépanneurs est le strict reflet de DepanTime** (étape 12, durcie le 2026-08-19). DepanTime tient les fiches de l'équipe, cette application n'en est qu'un consommateur — rien n'est jamais renvoyé vers DepanTime, et **rien ne se crée ni ne se modifie ici** :
  ...
  - **Réconciliation des fiches absentes de DepanTime** (y compris celles créées à la main avant que la synchro existe, donc sans clé externe) : celle qui **porte des documents** est archivée et conservée — les pièces de conformité doivent rester consultables après le départ ; celle qui **n'en porte aucune** est **supprimée**, il n'y a rien à garder et la liste doit rester le reflet exact de DepanTime. Le détail remonte dans `hors_depantime`.
